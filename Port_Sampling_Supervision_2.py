...
if uploaded_file is not None:
    try:
        ...
        column_mappings = {
            ...
            "ACTUAL SAMPLING TIMESTAMP": "BAG_AHK_LP_SAMPLING_CHECK_TS",
            "Added Email ID": "PRN_SAMP_CHECK_USER"
        }
        ...
        for original_col, new_col in column_mappings.items():
            if original_col in combined_df_for_download.columns:
                ...
                mapped_df_for_download[new_col] = None

        # Add "+02:00" to all time columns in mapped_df_for_download
        for col in ["BAG_AHK_LP_SAMPLING_CHECK_TS"]:
        ...
        file_name = f"From_{start_date.strftime('%Y%m%d')}_{start_time.strftime('%H%M')}_to_{end_date.strftime('%Y%m%d')}_{end_time.strftime('%H%M')}_Durban_Sampling.csv"
